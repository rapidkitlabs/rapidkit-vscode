...
type SharedParitySnapshot = {
  ...
  frameworkToStack: Record<string, DetectedStack>;
  runtimeToStack: Record<string, DetectedStack>;
};
...
describe('shared import stack parity snapshot (extension)', () => {
  ...
  it('matches framework-to-stack contract snapshot', () => {
    ...
    const canonicalActual = Object.fromEntries(
      EXPECTED_FRAMEWORK_KEYS.map((rawKey) => {
        const canonical = normalizeBackendFrameworkLabel(rawKey);
        return [
          rawKey,
          detectStackFromBackendHints({ framework: canonical, confidenceLevel: 'high' }).stack,
        ];
      })
    );
  ...
  it('matches runtime-hint-to-stack contract snapshot', () => {
    ...
    const canonicalActual = Object.fromEntries(
      EXPECTED_RUNTIME_HINT_KEYS.map((runtimeHint) => [
        runtimeHint,
        detectStackFromBackendHints({ runtime: runtimeHint, confidenceLevel: 'high' }).stack,
      ])
    );
